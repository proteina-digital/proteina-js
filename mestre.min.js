console.log("VERSÃO JS 2.8");function desabilitar(){alert("Todos os direitos Reservados. A cópia e reprodução não-autorizada está expressamente proibida.")
return!1}
...
var formatter=new StringMask(formato);var tel_atual_formatado=formatter.apply(tel_atual_clean);var tel_novo_formatado=formatter.apply(tel_novo_clean);console.log(tel_novo_clean);console.log(tel_atual_clean);$("a:contains('"+tel_atual_formatado+"')").each(function(){var current_html=$(this).html();$(this).html(current_html.replace(tel_atual_formatado,tel_novo_formatado));$(this).attr('href','tel:'+tel_novo_clean)});$('a[href*="'+tel_atual_clean+'"]').each(function(){$(this).attr('href','tel:'+tel_novo_clean)})};function checa_parametros_url(campanha_organica,c2c){cria_cookie("c2c",_GETURL("c2c")?_GETURL("c2c"):readCookie("c2c")?readCookie("c2c"):c2c);cria_cookie("site",_GETURL("utm_campaign")?_GETURL("utm_campaign"):readCookie("utm_campaign")?readCookie("utm_campaign"):campanha_organica);cria_cookie("utm_campaign",_GETURL("utm_campaign")?_GETURL("utm_campaign"):readCookie("utm_campaign")?readCookie("utm_campaign"):campanha_organica);cria_cookie("utm_source",_GETURL("utm_source")?_GETURL("utm_source"):readCookie("utm_source")?readCookie("utm_source"):'google');cria_cookie("utm_medium",_GETURL("utm_medium")?_GETURL("utm_medium"):readCookie("utm_medium")?readCookie("utm_medium"):'organic');_CRIAINPUT("site",readCookie("c2c"));_CRIAINPUT("site",readCookie("utm_campaign"));_CRIAINPUT("utm_source",readCookie("utm_source"));_CRIAINPUT("utm_medium",readCookie("utm_medium"));_CRIAINPUT("utm_campaign",readCookie("utm_campaign"));console.log('checa_parametros_url')}
function acao_pelo_tel(tel_atual,ddd,formato,campanha_organica,c2c){if(readCookie('telefone')){if(_GETURL("telefone")){if(readCookie('telefone')==_GETURL("telefone")){muda_tel(tel_atual,_GETURL("telefone"),ddd,formato);checa_parametros_url(campanha_organica,c2c);console.log("A")}else{muda_tel(tel_atual,_GETURL("telefone"),ddd,formato);cria_cookie("telefone",_GETURL("telefone"));checa_parametros_url(campanha_organica,c2c);console.log("B")}}else{muda_tel(tel_atual,readCookie("telefone"),ddd,formato);checa_parametros_url(campanha_organica,c2c);console.log("C")}}else{if(_GETURL("telefone")){muda_tel(tel_atual,_GETURL("telefone"),ddd,formato);cria_cookie("telefone",_GETURL("telefone"));checa_parametros_url(campanha_organica,c2c);console.log("D")}else{checa_parametros_url(campanha_organica,c2c)}}}
...
function carregar_fontes(){var body=$('body');if(body.hasClass('not_ready')){body.removeClass('not_ready')}}